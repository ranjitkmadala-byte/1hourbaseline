...
if entries.empty:
    ...
    v["TARGET +0.5%"]=pd.to_numeric(v["target_price"],errors="coerce").round(2)
    v["TARGET"]=v["target_hit"].map({True:"✅ HIT",False:"ACTIVE"})
    ...
    v["OPTION STATUS"]=v["5M SCORE"].apply(
        lambda x: "🔥 STRONG" if pd.notna(x) and x >= 3
        else ("WAIT" if pd.isna(x) else "NORMAL")
    )

    st.dataframe(
        v[["symbol","ENTRY TIME","ENTRY","AVWAP","SUPPLY",
           "5M SCORE","5M PROXY","15M SCORE","15M PROXY",
           "OPTION STATUS","TARGET +0.5%","TARGET"]]
...
if pipeline.empty:
    ...
    st.dataframe(
        pipeline[["symbol","STATUS","Last 3m","3m Close","AVWAP",
                  "Supply High","Breakout Close","Entry","5m Score","15m Score","Target"]],
        use_container_width=True,
        hide_index=True
    )
...
11. Signed-volume proxy is also shown. It is a research proxy, **not true CVD**.
""")
